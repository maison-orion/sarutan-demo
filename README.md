...
| 単語ページ | 中央＝単語・例文・意味・発音・動画／四隅＝品詞の違い・類義語・コアイメージ・解説／サル＝おさるさんの覚え方 | 四隅をタップ、または1本指で斜めに引っぱると（Google Maps 式・反対側の角が開く・引いている間は開く角が光る）ポップアップ。左下のコアイメージだけ別ページ（Word Sphere：中央にコアイメージ、周りに意味ごとの例文4つ）。サルは方言設定に合わせた覚え方。ほぼ真横に払うか、左右の矢印で前後の単語（矢印はスマホでも表示） |
...
- v0.5.2（2026-09-24）単語ページの前後の矢印をスマホでも表示（先方指摘。マウスを乗せられる端末だけに出していた）。単語の文字サイズを矢印と重ならない大きさに
- v0.5.1（2026-09-23）単語ページの四隅を「2本指」ではなく「1本指で斜めに引っぱる」で開くように変更（先方依頼）。タップで開く動きはそのまま。2本指とトラックパッドの操作は廃止
