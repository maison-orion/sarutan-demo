...
| TikTak | TikTokのように時間で次の単語が表示されるフラッシュカード・UIはTikTokオマージュ・動画3秒→自動で次へ | 縦フィード。単語の動画が3秒流れて次の単語へ自動で切り替わる（1.4秒で意味が出る）。タップで一時停止、上下スワイプで前後、右の列に発音／保存／詳しく（単語ページ）。ランダム／頻出順 |
